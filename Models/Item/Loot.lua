...
function ItemAward:initialize(entry, candidate, reason)
	if not entry or not entry:isInstanceOf(LootAllocateEntry) then
	...
	-- the name of the award_scaling reason
	self.awardReason = awardReason
	-- the scaling percentage associated with the awardReason
