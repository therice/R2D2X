    ...
    describe("functional", function()
        ...
        describe("sends", function()
            ...
            it("via GetSender", function()
                ...
                assert.spy(onReceiveSpy).was_called_with(match.is_table(), "Sender-Realm1", "test_cmd", C.Channels.Raid)
            ...
            describe("in different group types", function()
                ...
                it("party", function()
                    ...
                    assert.spy(onReceiveSpy).was_called_with(match.is_table(), "Sender-Realm1", "test_cmd", C.Channels.Party)
                end)
                it("guild", function()
                    ...
                    assert.spy(onReceiveSpy).was_called(1)
                    assert.spy(onReceiveSpy).returned_with()
                    assert.spy(onReceiveSpy).was_called_with(match.is_table(), "Sender-Realm1", "test_cmd", C.Channels.Guild)
                end)
                it("none", function()
                    Comm:GetSender(C.CommPrefixes.Main)(C.group, "test_cmd")
                    WoWAPI_FireUpdate(GetTime()+10)
                    assert.spy(onReceiveSpy).was_called(1)
                    assert.spy(onReceiveSpy).returned_with()
                    assert.spy(onReceiveSpy).was_called_with(match.is_table(), "Sender-Realm1", "test_cmd", C.Channels.Whisper)
                end)
            end)
