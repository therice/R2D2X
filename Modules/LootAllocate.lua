...
do
	...
	for index, award in ipairs(NonUserVisibleAwards) do
		Util.Tables.Insert(AwardReasons, index,
		                   {
			                   color       = AwardScaling[award].color,
			                   sort        = sortLevel,
			                   text        = L[award],
			                   award_scale = award,
			                   disenchant  = Util.Strings.StartsWith(Util.Strings.Lower(AwardScaling[award].text), 'disenchant')
		                   }
		)
	...
	if AddOn:IsMasterLooter() then
		...
		if ML:GetDbValue('announceResponses') then
			...
			if candidateResponse and tonumber(candidateResponse.response) ~= nil then
				...
				local subject = AddOn:StandingsModule():GetEntry(candidate)
				...
				local announcement = format(text,
				                            AddOn.Ambiguate(candidate),
				                            (subject and subject:GetPR() or 0.0),
