...
function Entry:Update(item)
	...
	if AddOn:MasterLooterDbValue('timeout') then
		self.timeoutBar:SetMinMaxValues(
			0,
			AddOn:MasterLooterDbValue('timeout') or AddOn:MasterLooterModule():GetDbValue('timeout')
		)
...
function Loot:GetFrame()
	if not self.frame then
		...
		local f = UI:NewNamed('Frame', UIParent, 'Loot', self:GetName(),  L["frame_loot"], 450, 375, false)
...
-- this function updates the GP portion of Item Text with values
-- based upon button response (and associated award scaling)
---@param entry Loot.Entry
---@param awardScale string
function Loot.UpdateItemText(entry, awardScale)
	local item = entry.item
	entry.itemLvl:SetText(
		"Level " .. item:GetLevelText() ..
		" " .. AddOn:GearPointsModule():GetGpTextColored(item, awardScale) ..
...
function Loot.UpdateItemResponders(entry, response)
	if entry and entry.item and response then
		...
		if responders and Util.Tables.Count(responders) > 0 then
			...
			if #text > 0 then
				UIUtil.CreateTooltip(unpack(text))
			end
		end
	end
end