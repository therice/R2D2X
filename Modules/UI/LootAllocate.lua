...
function LA:GetFrame()
	if not self.frame then
		RightClickMenu = MSA_DropDownMenu_Create(C.DropDowns.AllocateRightClick, UIParent)
		...
		state:SetPoint("LEFT", gp, "RIGHT", 5, 0)
		...
		-- abort button
		local abort = UI:NewNamed('Button', f.content)
		...
		-- filter
		local filter = UI:NewNamed('Button', f.content)
		...
		filter:SetScript("OnEnter", function() UIUtil.CreateTooltip(L["deselect_responses"]) end)
		filter:SetScript("OnLeave", function() UIUtil:HideTooltip() end)
		f.filter = filter

		-- disenchant
		local disenchant = UI:NewNamed('Button', f.content)
		...
		disenchant:SetScript("OnEnter", function() UIUtil.CreateTooltip(L["enchanter_select"]) end)
		disenchant:SetScript("OnLeave", function() UIUtil:HideTooltip() end)
...
function LA:UpdateSessionButton(session, texture, link, awarded)
	...
	btn:SetScript("OnEnter", function() UIUtil.CreateTooltip(unpack(lines)) end)
	return btn
end
	...
	if level == 1 then
		...
		for name in AddOn:GroupIterator() do
			...
			if player.enchanter then
				info.text =
					UIUtil.ClassColorDecorator(player.class):decorate(
							AddOn.Ambiguate(player:GetName())
