...
--- @type GearPointsCustom
local GpCustom = AddOn:GetModule("GearPointsCustom", true)

function GpCustom:GetAddItemFrame()
	if not self.addItemFrame then
		local f = UI:NewNamed('Frame', UIParent, 'AddCustomItem', 'GearPointsCustom', L['frame_add_custom_item'], 225, 200, false)
		...
		f.Query = function()
			...
			local function query(id)
				...
				Logging:Trace(
						...
						tostring(subType), tostring(equipLoc), tostring(GpCustom.EquipmentLocations[equipLoc])
				)
				if name then
					...
					if not GpCustom.EquipmentLocations[equipLoc] then equipLoc = "CUSTOM_SCALE" end
...
function GpCustom.DeleteItemOnClickYes(_, item)
	Logging:Debug("DeleteItemOnClickYes(%s)", tostring(item))
	GpCustom:RemoveItem(item)
end