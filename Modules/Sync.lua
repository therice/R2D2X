...
function Sync:OnSyncAccept(data)
	...
	self.frame.statusBar.Update(nil, _G.RETRIEVING_DATA)
end
...
function Sync:SendSyncData(target, type)
	...
	self:SendBulk(target, C.Commands.Sync, type, Sync:GetStream(target:GetName())[type])
...
function Sync:SubscribeToComms()
	...
	Comm():BulkSubscribe(C.CommPrefixes.Sync, {
